...
#define SAMPLE_HZ       25      // 1|10|25|50|100|200|400 Hz
#define TX_INTERVAL_MS  100        // ms between BLE UART sends
...
void setup() {
  ...
  // LED
  Bluefruit.autoConnLed(true);             // we control the LED
...
// ── Loop ────────────────────────────────────────────────
void loop() {
  // Echo any BLE UART input to Serial
  while (bleuart.available()) {
    Serial.write((uint8_t)bleuart.read());
  }

  // Echo any Serial input to BLE UART (useful for remote commands)
  ...
  if (!sensorOK || !Bluefruit.connected()) return;
...
void connect_callback(uint16_t conn_handle) {
  ...
  if (LED_ENABLED) digitalWrite(LED_BLUE, LOW);  // LOW = on
}
